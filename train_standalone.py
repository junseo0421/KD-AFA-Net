        ...
        for batch_idx, (gt, mask_img) in enumerate(train_loader):
            ...
            for _ in range(2):
                ...
                mrf_loss = mrf((mask_pred.cuda(0) + 1) / 2.0,
                               (iner_img.cuda(0) + 1) / 2.0) * 0.5 / batchSize
...
if __name__ == '__main__':
    ...
    def get_args():
        ...
        parser.add_argument('--name_model', type=str, choices=['teacher', 'student'],
                            default='teacher', help='the name of the model')
    ...
    if name_model == 'teacher':
        gen = AFA_Net(n_channels=3, n_classes=3).cuda()
    else:
        gen = LAFA_Net(n_channels=3, n_classes=3).cuda()
